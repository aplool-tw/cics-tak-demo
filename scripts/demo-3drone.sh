#!/usr/bin/env bash
# demo-3drone.sh — Start the full TAK pipeline (7 services) and open browser maps.
...
preflight() {
    ...
    local port=""
    for port in 8089 8090 8092 8093 18080 7070; do
...
print_banner() {
    printf '%s\n' "${BOLD}"
    echo "  +=================================================================+"
    echo "  |  TAK pipeline demo (three-drone)                               |"
    echo "  |  map-sim → uds → echoshield → sentrycs → gw → relay → tak-client |"
    echo "  +=================================================================+"
    printf '%s\n' "${RESET}"
    echo
    log "Three-drone staggered scenario (TRK-E01 / TRK-E02 / TRK-E03):"
    echo "  TRK-E01: 24.757282N,121.033750E  35 m/s heading 180°"
    echo "  TRK-E02: 24.754419N,121.002238E  25 m/s heading 135°"
    echo "  TRK-E03: 24.749968N,121.060359E  22 m/s heading 225°"
    echo "  t=  9s : TRK-E01 enters EchoShield range"
    echo "  t= 27s : TRK-E03 enters EchoShield range"
    echo "  t= 52s : TRK-E02 enters EchoShield range"
    echo "  t= 71s : TRK-E01 MITIGATING → takeover to HP"
    echo "  t=127s : TRK-E03 MITIGATING → takeover to HP"
...
launch_services() {
    ...
    log "PIDs: map=${MAPSIM_PID} uds=${UDS_PID} echo=${ECHO_PID} sntr=${SNTR_PID} gw=${GW_PID} relay=${RELAY_PID} tak=${TAK_CLIENT_PID}"
    log "Logs: ${LOG_DIR}/"
}
