...
- 每個主迴圈週期以 `POST /objects/update` 主動推送全量狀態到 Map Simulator（`:8090`），供 EchoShield Simulator 與 Sentrycs Simulator 透過 Map Simulator 查詢取得。
...
### User Story 2 — Sentrycs Simulator 查詢與接管（Priority: P1）

Sentrycs Simulator 作為 RF 偵測模擬器，需要：（a）透過 Map Simulator 取得無人機位置（非由此 UDS 查詢，但 Sentrycs 仍會透過 UDS 驗證特定 `drone_id` 的狀態與型號資訊）；（b）在內部狀態機進入「下令接管」時，直接呼叫 UDS 發出接管指令。

**Why this priority**：沒有這條路徑，接管閉環無法觸發。這也是 UDS `:8080` REST API 的主要消費者介面。

**Independent Test**：以 `curl` 或 Sentrycs Simulator 的單元測試，針對一個已知的 `drone_id` 分別呼叫 `GET /status/{drone_id}`、`GET /drones`、`POST /command/takeover`，驗證回應格式與 HTTP 狀態碼符合 ICD-UDS-API。

**Acceptance Scenarios**:

1. **Given** UDS 已載入場景且 `TRK-001` 存在，**When** 呼叫 `GET /status/TRK-001`，**Then** 回傳 200 OK 且 JSON 內含 `drone_id`、`lat`、`lon`、`alt_m`、`velocity_ms`、`flight_state`、`is_landed` 等欄位。
2. **Given** 要求接管的 `drone_id` 不存在，**When** 呼叫 `POST /command/takeover`，**Then** 回傳 400 並包含錯誤原因字串（`drone_id not found` 或 `already landed`）。
3. **Given** 場景含多架無人機，**When** 呼叫 `GET /drones`，**Then** 回傳所有活躍無人機的 `drone_id`、`model`、`flight_state` 清單。
...
1. **Given** UDS 與 Map Simulator 都已啟動，**When** UDS 完成一次主迴圈更新，**Then** UDS 必須呼叫一次 `POST http://<map-sim>:8090/objects/update`，Request body 必須包含所有 `flight_state ≠ IDLE` 的無人機。
2. **Given** UDS 的主迴圈頻率設為 10 Hz，**When** 持續執行 10 秒，**Then** 期間 `POST /objects/update` 的實際呼叫次數必須 ≥ 95 且 ≤ 105（允許 ±5 次抖動）。
3. **Given** 無人機進入 `LANDED`，**When** UDS 處理該無人機的下一個迴圈週期，**Then** UDS 必須在推送中包含該無人機最後一筆狀態（標示為已降落/LOST 語意），之後的迴圈週期不再推送該 `drone_id`。

---

### Edge Cases

- **無效 `track_id` / `drone_id` 接管**：`POST /command/takeover` 指向不存在的 `drone_id` → 400 `drone_id not found`；對已 `LANDED` 的無人機再發接管 → 400 `already landed`。
- **LANDED 後再接管**：系統必須拒絕且不改變狀態（避免降落中的殭屍無人機重新起飛）。
- **多架同時接管**：在同一迴圈週期內收到多筆不同 `drone_id` 的接管指令時，必須各自套用而互不干擾，每筆均獨立進入 `MITIGATING_TAKEOVER`。
- **對同一 `drone_id` 連續接管**：在尚未 `LANDED` 前重複呼叫 → 以最後一次指令的降落點為準（覆蓋 `takeover_cmd`）。
...
- **FR-UDS-002**：UDS 必須在每個主迴圈週期向 Map Simulator 發送一次 `POST http://<map-sim-host>:8090/objects/update`，request body 涵蓋所有活躍無人機的最新狀態（WGS84 位置、高度、速度、航向、`flight_state`、型號、操控者位置）。
- **FR-UDS-003**：UDS 必須以 HTTP REST Server 於 `:8080` 提供：
  - `GET /status/{drone_id}` — 回傳指定無人機當前狀態。
  - `GET /drones` — 回傳所有活躍無人機的摘要清單。
  - `POST /command/takeover` — 接收接管指令。
...
- **FR-UDS-006**：UDS 必須在高度 ≤ 2 m 且速度 ≤ 0.5 m/s 時自動將無人機切換為 `LANDED`，並在接下來的推送中標示該無人機為已降落（或於下一週期起不再包含該 `drone_id`）。
- **FR-UDS-007**：UDS 必須從 YAML 場景檔載入無人機清單、初始位置 / 高度 / 速度 / 航向、操控者位置、航點（waypoints）、降落點（`landing_point`）、`update_hz` 與時間軸事件（`timeline`）。
...
- **FR-UDS-011**：UDS CLI 必須至少提供 `--scenario`（YAML 路徑）、`--api-port`（預設 8080）、`--map-sim-url`（預設 `http://127.0.0.1:8090`）、`--hz`、`--verbose`。
...
- **FR-UDS-015**：UDS 必須拒絕下列無效的接管指令並回傳 HTTP 400：
  - `drone_id` 不存在於當前場景。
  - 目標無人機已處於 `LANDED`。
  - Request body 缺少 `drone_id` / `target_lat` / `target_lon` 或欄位型別錯誤。
...
> - 原 FR-UDS-014「LANDED 後自動停止 EchoShield TCP 廣播（最後一筆 track_status: LOST）」的「廣播 / track_status」語意已改為 FR-UDS-006 與 FR-UDS-014 的「推送 / 標示已降落」語意（`track_status` 由 EchoShield Simulator 產生，非 UDS）。
...
- **TakeoverCommand**：`drone_id`、`target_lat`、`target_lon`、`target_alt_m`（通常 0）、`descent_speed_ms`（預設 3.0 m/s）。來源為 `POST /command/takeover` 的 request body。
...
- **Scenario YAML Schema**（由 `ScenarioLoader` 讀取）：
  ...
  - `scenario.timeline[]`：事件清單，每個含 `at_s`、`action`（如 `start_flying`）、`drone_id`。
...
- **SC-001（飛行更新頻率）**：當 `update_hz` 設為 10 時，UDS 在穩態下對 Map Simulator 的 `POST /objects/update` 呼叫頻率必須達 10 ± 0.5 Hz（以 10 秒窗口量測）。
...
- **SC-004（多架無人機）**：UDS 必須可同時模擬至少 10 架無人機且維持 SC-001 的更新頻率；10 秒內推送總筆數（每架 × 每週期）偏差不超過 5%。
...
- **SC-006（GET /status 資料新鮮度）**：`GET /status/{drone_id}` 回傳的 `lat/lon/alt_m/velocity_ms/flight_state` 必須來自最近一個已完成的主迴圈週期，時間誤差 ≤ 1 × 主迴圈週期。
...
- **Sentrycs 位置查詢由 Map Simulator 承接**：UDS 的 `GET /status/{drone_id}` 主要供 Sentrycs Simulator 內部狀態機校驗與除錯用途，非日常位置更新來源。
