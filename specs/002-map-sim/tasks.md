...
- [X] T001 建立 `services/map-sim/` 目錄樹（src/map_sim/{api,cleanup,geo,models,registry}/、
  tests/{contract,integration,unit}/），各子目錄放入空 `__init__.py`（對齊 plan.md §Project Structure）。
- [X] T002 撰寫 `services/map-sim/pyproject.toml`：`[project]` name=`map-sim`、python>=3.11、依賴
...
- [X] T006 [P] [US1] 撰寫 `tests/contract/test_update_contract.py::test_update_happy_path`：
...
- [X] T007 [P] [US1] 撰寫 `tests/contract/test_update_contract.py::test_update_overwrites_same_drone`：
  對同 `drone_id` 先後送兩筆不同 `lat/lon/status` → 第二筆完全覆寫；`last_seen_at` 重設（US1
  Acceptance 2、Edge Case「重複 drone_id 更新」）。
- [X] T008 [P] [US1] 撰寫 `tests/contract/test_update_contract.py::test_update_accepts_extra_fields`
...
- [X] T009 [P] [US1] 撰寫 `tests/contract/test_update_contract.py::test_update_missing_required_field`
  —— 參數化 8 個欄位，逐一缺漏 → 400、`reason == "missing required field: <name>"`；**registry 不變**
  （FR-MS-015 副作用隔離；US1 Acceptance 3、Edge Case「缺必填欄位的 POST」）。
- [X] T010 [P] [US1] 撰寫 `tests/contract/test_update_contract.py::test_update_invalid_type`：
  數值欄位給字串、`status` 為空字串、`drone_id` 為空字串 → 400 `reason == "invalid type: <name>"`
  （contracts §3.1.2.2）。
- [X] T011 [P] [US1] 撰寫 `tests/contract/test_update_contract.py::test_update_invalid_timestamp`：
  `timestamp` 為 `"not-a-timestamp"` 或缺時區 → 400 `reason == "invalid type: timestamp"`
  （Edge Case「非法 timestamp 格式」）。
- [X] T012 [P] [US1] 撰寫 `tests/contract/test_update_contract.py::test_update_invalid_json`：
  body 為 `"{not json"` → 400 `reason == "invalid json"`（contracts §5）。
- [X] T013 [P] [US1] 撰寫 `tests/contract/test_update_contract.py::test_update_landed_is_accepted`：
  `status="LANDED"` → 200；物件仍保留於 registry、`status` 不被改寫（US1 Acceptance 5、FR-MS-014 前半）。
- [X] T014 [P] [US1] 撰寫 `tests/contract/test_update_contract.py::test_update_400_does_not_mutate_registry`
...
- [X] T015 [P] [US2] 撰寫 `tests/contract/test_query_contract.py::test_query_happy_path_radar_radius`：
...
- [X] T016 [P] [US2] 撰寫 `tests/contract/test_query_contract.py::test_query_sentrycs_radius`：
  同一資料集 `radius_m=8000` → 3 筆全出、升冪排序（US2 Acceptance 2）。
- [X] T017 [P] [US2] 撰寫 `tests/contract/test_query_contract.py::test_query_empty_result_returns_200_count_0`：
  查詢中心在南太平洋 → 200、`count==0`、`objects==[]`；**不得**回 404（US2 Acceptance 3、FR-MS-004、
  Edge Case「未知 drone_id 的查詢」）。
- [X] T018 [P] [US2] 撰寫 `tests/contract/test_query_contract.py::test_query_unknown_drone_id_empty`：
  registry 空 → `GET /objects?lat=0&lon=0&radius_m=1000` → 200、count=0（Edge Case「Map Sim 啟動
  時登錄表為空」）。
- [X] T019 [P] [US2] 撰寫 `tests/contract/test_query_contract.py::test_query_include_lost_false_hides_lost`
  —— 以 freezegun 讓一物件進入 `[ttl_warn_s, ttl_remove_s)` lost 區間，**未帶** `include_lost` 或
  `include_lost=false` → 該物件不出現（US2 Acceptance 4、FR-MS-006）。
- [X] T020 [P] [US2] 撰寫 `tests/contract/test_query_contract.py::test_query_include_lost_true_preserves_status`
...
- [X] T021 [P] [US2] 撰寫 `tests/contract/test_query_contract.py::test_query_is_lost_explicit_false`：
  `include_lost=false` 正常情境下，回應中所有 `objects[].is_lost` 必為 `false`（明確輸出、非省略；
  contracts §3.2.2.1）。
- [X] T022 [P] [US2] 撰寫 `tests/contract/test_query_contract.py::test_query_object_after_ttl_remove_hidden`
...
- [X] T023 [P] [US2] 撰寫 `tests/contract/test_query_contract.py::test_query_missing_required_parameter`：
  分別缺 `lat` / `lon` / `radius_m` → 400、`reason == "missing required parameter: <name>"`（FR-MS-005、
  US2 Acceptance 5、contracts §3.2.2.2）。
- [X] T024 [P] [US2] 撰寫 `tests/contract/test_query_contract.py::test_query_invalid_type`：
  `lat=abc` / `radius_m=xyz` / `include_lost=maybe` → 400 `reason == "invalid type: <name>"`
  （research.md §5、contracts §3.2.2.2）。
- [X] T025 [P] [US2] 撰寫 `tests/contract/test_query_contract.py::test_query_radius_non_positive`：
  `radius_m=0` 與 `radius_m=-1` → 400、`reason == "radius_m must be > 0"`（US2 Acceptance 6、
  Edge Case「radius_m = 0 或負值」）。
- [X] T026 [P] [US2] 撰寫 `tests/contract/test_query_contract.py::test_query_invalid_coordinates`：
  `lat=91` / `lon=181` → 400、`reason == "invalid coordinates"`（contracts §3.2.2.2）。
- [X] T027 [P] [US2] 撰寫 `tests/contract/test_query_contract.py::test_query_radius_extreme_large`：
  `radius_m=2e7`（>地球半周長）→ 200、回傳所有活躍物件、仍按距離升冪（Edge Case「radius_m 極大值」）。
- [X] T028 [P] [US2] 撰寫 `tests/contract/test_query_contract.py::test_query_include_lost_case_insensitive`：
  `include_lost=TRUE` / `True` / `false` → 皆接受；`include_lost=yes` → 400 `invalid type`（research.md §5）。
- [X] T029 [P] [US2] 撰寫 `tests/contract/test_query_contract.py::test_query_unknown_params_ignored`：
...
- [X] T030 [P] 撰寫 `tests/contract/test_health_contract.py::test_health_ok`：
  `GET /health` → 200、`status=="ok"`、`registered_objects` 為整數、`uptime_s` 為非負 float（FR-MS-010、
  contracts §3.3）。
- [X] T031 [P] 撰寫 `tests/contract/test_health_contract.py::test_health_registered_objects_matches_registry`：
  連推 3 筆不同 `drone_id` → `registered_objects == 3`（含 lost，對齊 `/objects/all` total）。
- [X] T032 [P] 撰寫 `tests/contract/test_health_contract.py::test_objects_all_totals`：
...
- [X] T033 [P] 撰寫 `tests/contract/test_health_contract.py::test_delete_drone_removed_and_not_found`：
...
- [X] T034 [P] [US3] 撰寫 `tests/integration/test_ttl_lifecycle.py::test_active_before_ttl_warn`：
  `ttl_warn_s=5, ttl_remove_s=10`；t=0 update → t=4.9 `GET /objects` 預設查詢見此物件、`is_lost=false`
  （US3 Acceptance 1 上半）。
- [X] T035 [P] [US3] 撰寫 `tests/integration/test_ttl_lifecycle.py::test_lost_between_warn_and_remove`：
...
- [X] T036 [P] [US3] 撰寫 `tests/integration/test_ttl_lifecycle.py::test_removed_after_ttl_remove`：
...
- [X] T037 [P] [US3] 撰寫 `tests/integration/test_ttl_lifecycle.py::test_landed_visibility_window`：
...
- [X] T038 [P] [US3] 撰寫 `tests/integration/test_ttl_lifecycle.py::test_update_resets_last_seen`：
  t=4 再 update 同 id → last_seen_at 重設；t=8 仍為 active（data-model.md §2 不變量）。
- [X] T039 [P] [US3] 撰寫 `tests/integration/test_ttl_lifecycle.py::test_cleanup_period_bound`：
...
- [X] T040 [P] [US3] 撰寫 `tests/integration/test_concurrent_safety.py::test_concurrent_post_and_query`：
  ...
  SC-MS-006）。
- [X] T041 [P] [US3] 撰寫 `tests/integration/test_concurrent_safety.py::test_cleanup_does_not_corrupt_query`：
  併發 POST + 手動快速呼叫 `cleanup_expired()`（覆寫極短 TTL）→ GET 永不回傳已被 delete 的 id、
  也不 raise（US3 Acceptance 4、FR-MS-007、data-model.md §3 鎖語意）。

### 3.3 Unit tests

- [X] T042 [P] 撰寫 `tests/unit/test_haversine.py`：對固定 10 組 `(lat1,lon1,lat2,lon2)`（含跨赤道、
  近極、短程），與 `geopy.distance.great_circle(..., radius=6371.0).meters` 交叉比對誤差 < 0.5%
  （SC-MS-005、research.md §1）。
- [X] T043 [P] 撰寫 `tests/unit/test_drone_object.py`：測 `DroneObject.age_s(now)` / `is_lost(ttl_warn_s,
...
- [X] T044 [P] 撰寫 `tests/unit/test_object_registry.py`：測 `update` 返回 `registered_at`；
...
- [X] T045 [US1] 撰寫 `tests/integration/test_uds_push_end_to_end.py::test_uds_payload_contract_compatibility`
...
- [X] T046 [P] 撰寫 `services/map-sim/src/map_sim/geo/haversine.py`：`haversine_m(lat1, lon1, lat2, lon2)
...
- [X] T047 [P] 撰寫 `services/map-sim/src/map_sim/models/drone_object.py`：`@dataclass DroneObject`
  ...
  → 讓 T043 綠。
- [X] T048 [P] 撰寫 `services/map-sim/src/map_sim/models/request.py`：pydantic v2 `UpdatePayload(BaseModel)`
...
- [X] T049 [P] 撰寫 `services/map-sim/src/map_sim/config.py`：`@dataclass(frozen=True) Settings(port: int
...
- [X] T050 [P] 撰寫 `services/map-sim/src/map_sim/logging.py`：`setup_logging(verbose: bool)` 初始化
  structlog JSON renderer，event key 約定見 research §7；風格對齊 `services/uds/src/uds/logging.py`。
- [X] T051 撰寫 `services/map-sim/src/map_sim/registry/object_registry.py`：`ObjectRegistry` 全實作
...
- [X] T052 撰寫 `services/map-sim/src/map_sim/cleanup/ttl_task.py`：
...
- [X] T053 撰寫 `services/map-sim/src/map_sim/api/errors.py`：
...
- [X] T054 撰寫 `services/map-sim/src/map_sim/api/server.py`：
...
- [X] T055 [US1] 撰寫 `services/map-sim/src/map_sim/api/handlers_update.py`：
...
- [X] T056 [US1] 於 `api/server.py` 註冊 `app.router.add_post("/objects/update", handlers_update.update)`。
...
- [X] T057 [US2] 撰寫 `services/map-sim/src/map_sim/api/handlers_query.py`：
  ...
  - 依賴 T047 / T051 / T053。
- [X] T058 [US2] 於 `api/server.py` 註冊 `app.router.add_get("/objects", handlers_query.query)`。
...
- [X] T059 [US3] 於 `api/server.py` 的 `on_startup` 啟動 `asyncio.create_task(run_cleanup_loop(registry,
...
- [X] T060 [US3] 在 `ObjectRegistry.query_radius` / `cleanup_expired` 的實作層面，針對「物件剛過
  `ttl_remove_s` 但背景 task 尚未觸發」的窗口，確保 `query_radius` 已先排除（data-model.md §3 註解；
  若 T051 已覆蓋則此為驗證 + 測試調整）。
- [X] T061 [US3] 針對 `settings.cleanup_period_s` 增加 config 覆寫路徑：`build_app(settings)` 接受
...
- [X] T062 [P] 撰寫 `services/map-sim/src/map_sim/api/handlers_admin.py::health`：
...
- [X] T063 [P] 撰寫 `handlers_admin.py::objects_all`：
...
- [X] T064 [P] 撰寫 `handlers_admin.py::delete_drone`：
  ...
  **注意：不存在也回 200，不回 404**）。
- [X] T065 於 `api/server.py` 註冊三個 admin 路由。讓 `tests/contract/test_health_contract.py` 全綠。

---

## Phase 10: CLI 與進入點

- [X] T066 撰寫 `services/map-sim/src/map_sim/cli.py`：argparse 僅接受 `--port`（default 8090）、
...
- [X] T067 撰寫 `services/map-sim/src/map_sim/__main__.py`：`from .cli import main; main()` 以支援
...
- [X] T068 [P] Review 全部 handler 確認 logging event key 與 research §7 對齊：`update.ok` /
...
- [X] T069 [P] 撰寫 `services/map-sim/tests/integration/test_side_effect_isolation.py`
  —— 跨 story regression：對 POST / GET 每個 400 分支各測一次，之後 `await registry.count()` 與
  `/objects/all` 狀態與該 400 前完全相同（FR-MS-015、SC-MS-006 邊界）。
- [X] T070 [P] 撰寫 `services/map-sim/tests/integration/test_ttl_precision.py`：
...
- [X] T071 [P] 撰寫 `services/map-sim/tests/unit/test_errors.py`：驗證 `errors.py` 所有 `reason` 字串
  常數與 `contracts/rest-api.md §5` 速查表**逐字相等**（未來若 implementation 誤改字串，契約測試 +
  此測試雙層擋）。
- [X] T072 [P] 執行 `quickstart.md §3` 的 curl 範例腳本化（`services/map-sim/scripts/smoke.sh` 或
  `tests/integration/test_quickstart_smoke.py`），驗證 §3.1 / §3.2 / §3.3 / §3.4 / §3.5 全部走得通。
- [X] T073 [P] 更新 `services/map-sim/README.md`：加入 quickstart 摘要 + `pytest` / `ruff check src
  tests` / `black --check src tests` 指令（對齊 `services/uds/README.md` 風格）。
- [X] T074 執行 `cd services/map-sim && ruff check src tests && black --check src tests && pytest`，全綠
  後視為 feature 完成。
