...
- [X] T043 [US1] 實作 `services/sentrycs-sim/src/sentrycs_sim/cli.py`：argparse `--scenario` / `--api-port` / `--verbose`；`main()` = 載入 config → configure_logging → 起 aiohttp runner :17070 → 起 loop task → signal handler → graceful shutdown 3s timeout（FR-SC-021..025）
